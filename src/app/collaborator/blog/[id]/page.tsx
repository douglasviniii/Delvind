...
export default function CollaboratorBlogPostPage() {
  ...
  const handleLike = async () => {
    if (!user) {
        toast({ title: 'Ação Requer Login', description: 'Você precisa estar logado para curtir um post.', variant: 'destructive'});
        return;
    }
    const postRef = doc(db, 'blog', id);
  ...
  const handleShare = () => {
      ...
      toast({ title: 'Link Copiado!', description: 'O link público do post foi copiado.' });
  }
  ...
  return (
    <article className="container max-w-4xl py-12">
        <div className='mb-8'>
            ...
            <Link href="/collaborator/feed-blog">
